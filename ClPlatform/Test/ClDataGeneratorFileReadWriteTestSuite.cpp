...
TEST_F( ClDataGeneratorFileReadWriteTestSuite, writeAndReadFloatArrayGenerator)
{
    ...
    boost::shared_ptr<IClDataGenerator> readGenerator;
...
{
    std::vector<boost::shared_ptr<IClDataGenerator> > generators;
    for ( int i=0; i < 10; i++ ) 
    {
        generators.push_back(boost::make_shared<ClRandomFloatArrayGenerator>(10+i));
    }
